...
CREATE TABLE ACHIEVEMENTS (
    ...
    RARITY          VARCHAR(50)     NOT NULL DEFAULT 'COMMON' CHECK (RARITY IN ('COMMON', 'RARE', 'EPIC', 'LEGENDARY', 'EXCLUSIVE')),
...
CREATE TABLE USER_PROFILES (
    ...
    CURRENT_LEVEL       INT             NOT NULL DEFAULT 1 REFERENCES LEVEL_CONFIG(LEVEL_ID),
    CURRENT_TITLE       VARCHAR(100)    NULL,
    GAMIFICATION_ENABLED BOOLEAN        NOT NULL DEFAULT FALSE,
    IS_NEWBIE_GIFT_CLAIMED BOOLEAN      NOT NULL DEFAULT FALSE,
    MAP_AVATAR_URL      TEXT            NULL,
    EQUIPPED_MAP_AVATAR_ID INT          NULL,
    LAST_ATTENDANCE_DATE DATE           NULL,
    ATTENDANCE_STREAK   INT             NOT NULL DEFAULT 0 CHECK (ATTENDANCE_STREAK >= 0),
    ...
    TOTAL_ACHIEVEMENTS  INT             NOT NULL DEFAULT 0 CHECK (TOTAL_ACHIEVEMENTS >= 0),
    UPDATED_AT          TIMESTAMPTZ     NOT NULL DEFAULT NOW()
);
...
CREATE TABLE LOCATIONS (
    ...
    BASE_REWARD_EXP INT             NOT NULL DEFAULT 10 CHECK (BASE_REWARD_EXP >= 0),
    BASE_REWARD_COIN INT            NOT NULL DEFAULT 10 CHECK (BASE_REWARD_COIN >= 0),
    CREATE_AT       TIMESTAMPTZ     NOT NULL DEFAULT NOW(),
    UPDATE_AT       TIMESTAMPTZ     NOT NULL DEFAULT NOW(),
...
CREATE TABLE REWARD_HISTORY_LOGS (
    LOG_ID          UUID            PRIMARY KEY DEFAULT gen_random_uuid(),
    USER_ID         UUID            NOT NULL REFERENCES USERS(USER_ID),
    SOURCE_TYPE     VARCHAR(50)     NOT NULL DEFAULT 'CHECKIN' CHECK (SOURCE_TYPE IN ('CHECKIN', 'LEVEL_UP', 'SYSTEM_GIFT', 'DAILY_ATTENDANCE')),
...
-- ============================================================
-- NHÓM 9: ADVANCED GAMIFICATION & EVENTS
-- ============================================================
...
CREATE TABLE VIRTUAL_ITEMS (
    ...
    ITEM_TYPE       VARCHAR(50)     NOT NULL CHECK (ITEM_TYPE IN ('MAP_AVATAR', 'VOUCHER', 'QUEST_KEY', 'TREASURE_CHEST')),
    ICON_URL        VARCHAR(500)    NULL,
    CREATED_AT      TIMESTAMPTZ     NOT NULL DEFAULT NOW()
);
...
CREATE TABLE USER_INVENTORY (
    USER_ID         UUID            NOT NULL REFERENCES USERS(USER_ID),
    ITEM_ID         INT             NOT NULL REFERENCES VIRTUAL_ITEMS(ITEM_ID),
    QUANTITY        INT             NOT NULL DEFAULT 0 CHECK (QUANTITY >= 0),
    UPDATED_AT      TIMESTAMPTZ     NOT NULL DEFAULT NOW(),
    PRIMARY KEY (USER_ID, ITEM_ID)
);

-- Thêm Foreign Key cho USER_PROFILES sau khi bảng VIRTUAL_ITEMS được tạo
ALTER TABLE USER_PROFILES ADD CONSTRAINT FK_USER_MAP_AVATAR FOREIGN KEY (EQUIPPED_MAP_AVATAR_ID) REFERENCES VIRTUAL_ITEMS(ITEM_ID);

-- ============================================================

CREATE TABLE DAILY_REWARD_CONFIG (
    DAY_STREAK      INT             PRIMARY KEY CHECK (DAY_STREAK > 0),
    REWARD_EXP      INT             NOT NULL DEFAULT 0 CHECK (REWARD_EXP >= 0),
    REWARD_COIN     INT             NOT NULL DEFAULT 0 CHECK (REWARD_COIN >= 0),
    REWARD_ITEM_ID  INT             NULL REFERENCES VIRTUAL_ITEMS(ITEM_ID),
    DESCRIPTION     VARCHAR(255)    NULL
);
...
CREATE TABLE MAP_TREASURES (
    ...
    LOCATION_ID     UUID            NULL REFERENCES LOCATIONS(LOCATION_ID),
    LATITUDE        NUMERIC(10, 6)  NOT NULL CHECK (LATITUDE >= -90 AND LATITUDE <= 90),
    LONGITUDE       NUMERIC(10, 6)  NOT NULL CHECK (LONGITUDE >= -180 AND LONGITUDE <= 180),
    CHECKIN_RADIUS  INT             NOT NULL DEFAULT 50,
    REWARD_ITEM_ID  INT             NULL REFERENCES VIRTUAL_ITEMS(ITEM_ID),
    REWARD_EXP      INT             NOT NULL DEFAULT 0 CHECK (REWARD_EXP >= 0),
    REWARD_COIN     INT             NOT NULL DEFAULT 0 CHECK (REWARD_COIN >= 0),
    IS_HIDDEN_QUEST BOOLEAN         NOT NULL DEFAULT FALSE,
...
CREATE TABLE USER_TREASURE_CLAIMS (
    USER_ID         UUID            NOT NULL REFERENCES USERS(USER_ID),
    SPAWN_ID        UUID            NOT NULL REFERENCES MAP_TREASURES(SPAWN_ID),
...
CREATE TABLE BUSINESS_CAMPAIGNS (
    ...
    EXP_MULTIPLIER  FLOAT           NOT NULL DEFAULT 1.0 CHECK (EXP_MULTIPLIER >= 1.0),
    COIN_MULTIPLIER FLOAT           NOT NULL DEFAULT 1.0 CHECK (COIN_MULTIPLIER >= 1.0),
...
CREATE TABLE VOUCHERS (
    ...
    CAMPAIGN_ID         UUID            NOT NULL REFERENCES BUSINESS_CAMPAIGNS(CAMPAIGN_ID),
    NAME                VARCHAR(255)    NOT NULL,
    DESCRIPTION         TEXT            NULL,
...
CREATE TABLE LEADERBOARD_RECORDS (
    ...
    USER_ID         UUID            NOT NULL REFERENCES USERS(USER_ID),
    CITY_ID         INT             NOT NULL REFERENCES CITIES(CITY_ID),
    PERIOD_TYPE     VARCHAR(20)     NOT NULL CHECK (PERIOD_TYPE IN ('WEEK', 'MONTH', 'ALL_TIME')),
    PERIOD_VALUE    VARCHAR(50)     NOT NULL,
    SCORE_EXP       INT             NOT NULL DEFAULT 0,
    SCORE_COIN      INT             NOT NULL DEFAULT 0,
    SCORE_DISTANCE  NUMERIC(10, 2)  NOT NULL DEFAULT 0,
    RANKING         INT             NULL,
    UPDATED_AT      TIMESTAMPTZ     NOT NULL DEFAULT NOW(),
    CONSTRAINT UQ_LEADERBOARD_RECORD UNIQUE (USER_ID, CITY_ID, PERIOD_TYPE, PERIOD_VALUE)
);
...
CREATE INDEX IF NOT EXISTS idx_leaderboard_coins ON USER_PROFILES(TOTAL_COINS_EARNED DESC);
CREATE INDEX IF NOT EXISTS idx_leaderboard_distance ON USER_PROFILES(TOTAL_DISTANCE DESC);
CREATE INDEX IF NOT EXISTS idx_leaderboard_achievements ON USER_PROFILES(TOTAL_ACHIEVEMENTS DESC);
