...
player_cooldowns: Dict[str, datetime] = {}
...
async def process_location_update(user_id: str, lat: float, lng: float):
    ...
    now = datetime.utcnow()
    if user_id in player_cooldowns and player_cooldowns[user_id] > now:
        return

    for other_user, loc in player_locations.items():
        if other_user in player_cooldowns and player_cooldowns[other_user] > now:
            continue
...
async def cancel_quest_instance(instance_id: str, reason: str = "Nhiệm vụ bị hủy"):
    ...
    # Thiết lập cooldown 3 phút để tránh bị spam ghép đôi ngay lập tức khi đứng gần
    cooldown_until = datetime.utcnow() + timedelta(minutes=3)
    player_cooldowns[p1] = cooldown_until
    player_cooldowns[p2] = cooldown_until
                ...
                if action == "update_location":
                    ...
                    if instance_id and instance_id in active_instances:
                        ...
                        if dist <= 10.0:  # Dung sai 10m an toàn cho GPS drift
                            ...
                            # Thiết lập cooldown 5 phút sau khi hoàn thành nhiệm vụ thành công
