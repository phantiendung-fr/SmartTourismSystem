...
# ---------------------------------------------------------------------------
# GET /api/v1/locations/daily-explore
# ---------------------------------------------------------------------------
import random
from cachetools import TTLCache

explore_cache = TTLCache(maxsize=5, ttl=3600)  # Cache 1 hour

@router.get("/locations/daily-explore", tags=["Locations"])
def get_daily_explore(db: Session = Depends(get_session)):
    """Lấy 5 địa điểm ngẫu nhiên mỗi ngày cho Vùng Đất Thám Hiểm."""
    all_locations = db.exec(
        select(Locations)
        .where(Locations.is_active == True)
        .where(Locations.deleted_at.is_(None))
    ).all()
    
    if not all_locations:
        return []

    # Dùng seed là ngày hiện tại
    today_seed = datetime.now().date().toordinal()
    
    if today_seed in explore_cache:
        return explore_cache[today_seed]

    rng = random.Random(today_seed)
    
    num_to_select = min(5, len(all_locations))
    selected_locs = rng.sample(all_locations, num_to_select)
    
    result = []
    for loc in selected_locs:
        image = db.exec(
            select(LocationsImage.url)
            .where(LocationsImage.location_id == loc.location_id)
            .order_by(LocationsImage.display_order.asc())
            .limit(1)
        ).first()
        
        # Tạo rating và phần thưởng ngẫu nhiên theo ngày và địa điểm
        loc_seed = today_seed + hash(loc.location_id)
        loc_rng = random.Random(loc_seed)
        
        rating = round(loc_rng.uniform(4.5, 5.0), 1)
        xp = loc_rng.randint(2, 10) * 100  # 200 - 1000 EXP
        coins = loc_rng.randint(10, 50) * 10 # 100 - 500 Xu
        difficulty = loc_rng.choice([('easy', 'Dễ'), ('medium', 'Trung bình'), ('hard', 'Khó')])
        ...
        result.append({
            ...
            "image": image or "https://images.unsplash.com/photo-1528127269322-539801943592?auto=format&fit=crop&w=600&q=80",
            "difficultyTone": difficulty[0],
    ...
    explore_cache[today_seed] = result
    return result
